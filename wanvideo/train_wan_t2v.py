...
class TextVideoDataset(torch.utils.data.Dataset):
    ...
    def load_hdf5(self, file_path):
        with h5py.File(file_path, 'r') as f:
            ...
            frames = self.load_frames_from_hdf5(data, start_frame_id, end_frame_id)
            text = self.load_text_from_hdf5(file_path, start_frame_id, end_frame_id)
            
            if self.is_i2v:
                frames, first_frame = frames
                return text, frames, first_frame    
            else:
                return text, frames
    ...
    def load_text_from_hdf5(self, file_path, start_frame_id, end_frame_id):
        with h5py.File(file_path, 'r') as f:
            ...
            if 'substep_reasonings' in f and f['substep_reasonings'].shape[0] > end_frame_id:
                texts = [f['substep_reasonings'][i].decode('utf-8').strip() for i in range(start_frame_id, end_frame_id)]
    ...
    def load_frames_from_hdf5(self, data, start_frame_id, end_frame_id):
    ...
    def __getitem__(self, data_id):
        ...
        if self.is_image(path):
            ...
            text, video, first_frame = self.load_hdf5(from_path)
            # hdf5 分散，需要根据文件名来确定保存路径, 统一保存到 base_path/train/file_name 下
        ...
        if self.is_i2v:
            data = {"text": text, "video": video, "path": to_path, "first_frame": first_frame}
        else:
