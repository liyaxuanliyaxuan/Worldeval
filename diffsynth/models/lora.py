...
class GeneralLoRAFromPeft:
    ...
    def get_name_dict(self, lora_state_dict):
        ...
        for key in lora_state_dict:
            ...
            # 新增 Action 投影层的键名处理
            if "action_proj" in key:
                # 示例键名转换： 
                # "denoising_model.action_proj.0.lora_B.weight" → "action_proj.0.weight"
                parts = key.split(".")
                module_path = ".".join(parts[parts.index("action_proj"):-2])  # 提取 "action_proj.0"
                target_name = f"{module_path}.weight"
    ...
    def match(self, model: torch.nn.Module, state_dict_lora):
        ...
        unmatched_keys = [key for key in lora_name_dict if key not in model_name_dict]
        if matched_num == len(lora_name_dict) or all(key.startswith("action_") for key in unmatched_keys):
            return "", ""
        else:
            return None
