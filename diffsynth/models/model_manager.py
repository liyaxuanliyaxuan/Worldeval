...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device, custom_params):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        combined_kwargs = {**extra_kwargs}
        if custom_params and model_name == "wan_video_dit":
            combined_kwargs.update(custom_params)
            if combined_kwargs != extra_kwargs:
                print(f"        Adding custom parameters: {custom_params}")
        ...
        if custom_params and "action_alpha" in custom_params:
            initialize_action_proj(model, custom_params["action_alpha"])
        ...
        if missing_keys:
            print(f"WARNING: miss {missing_keys}")
        if unexpected_keys:
...
class ModelDetectorFromSingleFile:
    def __init__(self, model_loader_configs=[], custom_params={}):
    ...
    def load(self, file_path="", state_dict={}, device="cuda", torch_dtype=torch.float16, **kwargs):
        ...
        if keys_hash_with_shape in self.keys_hash_with_shape_dict:
            ...
            loaded_model_names, loaded_models = load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device, self.custom_params)
            return loaded_model_names, loaded_models
...
class ModelManager:
    def __init__(
        ...
        self.model_detector = [
            ModelDetectorFromSingleFile(model_loader_configs, custom_params),
            ModelDetectorFromSplitedSingleFile(model_loader_configs, custom_params),
            ModelDetectorFromHuggingfaceFolder(huggingface_model_loader_configs, custom_params),
            ModelDetectorFromPatchedSingleFile(patch_model_loader_configs, custom_params),
        ]
    ...
    def load_model_from_single_file(self, file_path="", state_dict={}, model_names=[], model_classes=[], model_resource=None):
        ...
        if len(state_dict) == 0:
            state_dict = load_state_dict(file_path)
        model_names, models = load_model_from_single_file(state_dict, model_names, model_classes, model_resource, self.torch_dtype, self.device, self.custom_params)
    ...
    def load_lora(self, file_path="", state_dict={}, lora_alpha=1.0):
        if isinstance(file_path, list):
            ...
            for model in self.model:
                if hasattr(model, "action_proj"):
                    print(f"    Adding LoRA to action_proj layers")
                    # Determine the layers to process based on the length of action_proj
                    layers_to_process = [0, 2] if len(model.action_proj) <= 3 else [0, 2, 4]
                    for i in layers_to_process:
                        if i >= len(model.action_proj):
                            continue
                        layer = model.action_proj[i]
                        # Construct the prefix for the current layer (e.g., action_proj.0)
                        prefix = f"action_proj.{i}"
                        
                        # Load original parameters
                        weight_key = f"{prefix}.base_layer.weight"
                        bias_key = f"{prefix}.base_layer.bias"
                        if weight_key in state_dict:
                            layer.weight.data.copy_(state_dict[weight_key])
                            print(f"Weight loaded correctly for {weight_key}")
                        if bias_key in state_dict and layer.bias is not None:
                        ...
                        if lora_A_key in state_dict and lora_B_key in state_dict:
                            layer.lora_A = torch.nn.Parameter(state_dict[lora_A_key])
                            layer.lora_B = torch.nn.Parameter(state_dict[lora_B_key])
                            print(f"      Successfully injected to layer {i}")
                        else:
                            print(f"      Missing keys for layer {i}: {lora_A_key}, {lora_B_key}")
                    
                    if hasattr(model, "action_alpha"):
                        # Load action_alpha from state_dict if available
                        action_alpha_key = "action_alpha"
                        if action_alpha_key in state_dict:
                            model.action_alpha = nn.Parameter(state_dict[action_alpha_key])
                            print(f"Loaded action_alpha from state_dict: {state_dict[action_alpha_key]}")
                        else:
                            model.action_alpha = nn.Parameter(torch.tensor(0.1))
                            print("Default action_alpha set to 0.1")
